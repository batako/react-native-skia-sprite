...
- [ ] `useImperativeHandle(ref, () => ({ play, stop, pause, resume, setFrame, isPlaying, getCurrentAnimation }))` を実装。  
- [ ] 新しい props: `onAnimationEnd?: (name: string) => void`, `onFrameChange?: (payload) => void`。  
- [ ] `frame change` は `useEffect` で frameIndex が変わる度に呼ぶ (無限ループ防止に注意)。
